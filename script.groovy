...
def buildJarwithcred(){
  ...
  sh 'mvn package jib:build -Djib.from.auth.username=${docker_user} -Djib.from.auth.password=${docker_pass} -Djib.to.auth.username=${qeema_user} -Djib.to.auth.password=${qeema_pass}'
  }
}
